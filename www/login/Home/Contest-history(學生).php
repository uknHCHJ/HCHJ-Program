  ...
  function updateStudentTable(data) {
    ...
    data.forEach(function(item) {  // 修正：將 `)` 移到這一行的結尾
        ...
        button.onclick = function() {
            var url = 'download_image.php?competition=' + encodeURIComponent(item.name);
        ...
        deleteButton.textContent = '刪除歷程';
        deleteButton.classList.add('btn', 'btn-danger');
        deleteButton.onclick = function() {
            if (confirm("確定要刪除此競賽歷程嗎？")) {
                deleteUser(item.user); // 發送刪除請求
            }
        };
...
document.getElementById('upload-record-btn').onclick = function() {
    window.location.href = 'upload_record.php'; // 替換為實際的上傳頁面
};
</script>
</body>
        </div>
