        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td>
                    <form action="downloadreview-02.php" method="POST">
                        ...
                        <button type="submit" class="download-button">查看備審</button>
                    </form>
                  </td>';
            echo '<td>
                    <form action="recordforreview-01(備審紀錄後端).php" method="POST">
                        <input type="hidden" name="file_id" value="' . $row['id'] . '">
                        <button type="submit" class="download-button">查看競賽</button>
                    </form>
                  </td>';
            echo '<td>
                    <form action="recordforreview-01(備審紀錄後端).php" method="POST">
                        <input type="hidden" name="file_id" value="' . $row['id'] . '">
                        <button type="submit" class="download-button">查看志願</button>
                    </form>
                  </td>';
            echo '</tr>';
        }
