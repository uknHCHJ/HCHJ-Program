...
$selectedGrade = trim($_GET['grade']); // 去除多餘的空格
$selectedClass = trim($_GET['class']);
// 防止 SQL 注入，轉義特殊字元
$selectedGrade = mysqli_real_escape_string($link, $selectedGrade);
$selectedClass = mysqli_real_escape_string($link, $selectedClass);

// 準備查詢語句，查詢特定 class 的資料
$query = "SELECT class, name, user FROM user WHERE class = '$selectedClass' AND grade = '$selectedGrade'";
