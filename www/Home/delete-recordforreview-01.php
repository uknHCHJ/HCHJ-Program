...
if (isset($_POST['file_id']) && isset($_POST['confirm_delete']) && $_POST['confirm_delete'] === 'yes') {
  ...
  if ($stmt->affected_rows > 0) {
    // 刪除成功
    echo "<script>alert('檔案已成功刪除'); window.location.href = '/~HCHJ/Home/recordforreview01-1.php';</script>";
  } else {
    // 刪除失敗
    echo "<script>alert('檔案不存在或刪除失敗'); window.location.href = '/~HCHJ/Home/recordforreview01-1.php';</script>";
  }
