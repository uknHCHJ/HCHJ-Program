...
if ($result_students) {
    while ($student = mysqli_fetch_assoc($result_students)) {
        ...
        $query = "SELECT MAX(upload_time) AS latest_upload, COUNT(*) AS upload_count 
                  FROM portfolio 
                  WHERE student_id='$student_id' AND category='實習證明'";
        ...
        $download_link = ($upload_count > 0) ? "<a href='teacher-download-02.php?id={$student_id}&category=成績單'>📂 下載</a>" : "";
