...
if ($result_students) {
    while ($student = mysqli_fetch_assoc($result_students)) {
        ...
        $query = "SELECT MAX(upload_time) AS latest_upload, COUNT(*) AS upload_count 
                  FROM portfolio 
                  WHERE student_id='$student_id' AND category='專業證照'";
        ...
        $download_link = ($upload_count > 0) ? "<a href='teacher-download-02.php?id={$student_id}&category=專業證照'>📂 下載 ZIP</a>" : "";
