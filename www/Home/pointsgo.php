...
<body>
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-6">
                <div class="container">
                    <h3><label for="certificateSelect">選擇證照:</label></h3>
                    ...
                    <!-- 按鈕和志願序列表 -->
                    <button onclick="add()">新增到清單</button>
                    <h2>證照(最多1個)</h2>
                    <ul id="preferenceList"></ul>
                    <button onclick="submit()">一鍵推薦</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
            ...
            fetch('getCertificate.php')
                ...
                .then(data => {
                    ...
                    data.forEach(certificate => {
                        ...
                        option.value = certificate.ID; // 使用證照的 ID 作為選項值
                        option.textContent = certificate.name; // 顯示證照名稱
        ...
        function add() {
            ...
            const preference = certificateSelect.options[certificateSelect.selectedIndex].text;

            // 檢查志願是否已經被選過
            if (preferences.length >= maxPreferences) {
                alert(`最多只能選擇${maxPreferences}個志願`);
                return;
            }
            ...
            preferences.push({
                order: order,
                certificateId: certificateSelect.value,
            ...
            fetch("addPreference.php", {
                ...
                body: JSON.stringify({
                    preferences: preferences.map((pref, index) => ({
                        ...
                        certificate_id: pref.certificateId,
                    })),
                }),
            })
                .then(response => response.json())
