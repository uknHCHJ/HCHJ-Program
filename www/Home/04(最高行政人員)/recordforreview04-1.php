...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 發送請求到後端取出資料
    fetch('recordforreview04-2.php?user=' + encodeURIComponent(userValue))
  ...
  function updateStudentTable(data) {
    ...
    data.forEach(function(item) {  // 修正：將 `)` 移到這一行的結尾
        ...
        uploadTimeCell.textContent = "第" +item.id + "次"; // 顯示上傳時間
        
        row.insertCell(1).textContent = item.upload_date;
        row.insertCell(2).textContent = item.name;
        var cell = row.insertCell(3);
        var button = document.createElement('button');
        button.textContent = '下載備審';
        button.classList.add('btn', 'btn-primary');

        button.onclick = function() {
            var url = 'recordforreview04-1 delete.php?user=' + encodeURIComponent(userValue);
            window.location.href = url; // 直接跳轉到指定 URL
        };
