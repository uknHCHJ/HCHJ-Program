...
if (in_array("4", $permissions)) {
    // 權限中有 4，從資料庫取得所有班級
    $gradeClassPairs = [];
    // 假設資料表名稱為 classes，且有 grade 與 class 欄位
    $sql = "SELECT DISTINCT grade, class FROM user";
    $result = mysqli_query($link, $sql);
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            // 例如將年級與班級組合成 "1A" 這樣的格式
            $gradeClassPairs[] = $row['grade'] . $row['class'];
        }
    }
} else {
    // 沒有權限 3，只顯示該使用者擁有的班級
              ...
              document.addEventListener("DOMContentLoaded", function () {
                ...
                classButtons.forEach(button => {
                  button.addEventListener("click", function () {
                    ...
                    // 取得後端資料
                    fetch("student04-2.php?class=" + selectedClass)
                      .then(response => {
