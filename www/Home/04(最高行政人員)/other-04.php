...
if ($result_students) {
    while ($student = mysqli_fetch_assoc($result_students)) {
        ...
        $query = "SELECT MAX(upload_time) AS latest_upload, COUNT(*) AS upload_count 
                  FROM portfolio 
                  WHERE student_id='$student_id' AND category='其他資料'";
        ...
        $download_link = ($upload_count > 0) ? "<a href='teacher-download-04.php?id={$student_id}&category=其他資料'>📂 下載</a>" : "";
